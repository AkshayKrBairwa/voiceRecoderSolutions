...
if(!$conn){
	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		if(isset($_FILES["recordedFile"]) && !$_FILES["recordedFile"]['error']){
			move_uploaded_file($_FILES["recordedFile"]['tmp_name'], "uploads/" . $fileName=date("YmdHis") . ".mp3");
			$result["error"] = 0;
 			$query= "INSERT INTO `heb`(`paper_id`, `question_id`, `audio_url`, `type`) VALUES ('1','1','$fileName','voice')";
...
<body>
	...
	<div class="container main-container">
		...
		<div class="center-child" id="parent-recorder">
			...
			<a  class="btn btn-primary" href="viewResult.php">
			    
			   	<i class="large material-icons">library_music</i>
			</a>
			...
			if(typeof(mediaRecorder) != 'undefined'){
				...
				if(action == 'record'){
					...
					mediaRecorder.onstop = function(e){
						var blob = new Blob(window.audioTracks, {type: 'audio/mp3; codecs=opus'});
